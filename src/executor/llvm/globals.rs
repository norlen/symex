...
pub struct GlobalReference {
    ...
    pub kind: GlobalReferenceKind,
}

/// Can hold either a [Function] or [GlobalVariable].
#[derive(Debug, Clone)]
pub enum GlobalReferenceKind {
    /// Holds the referenced [GlobalVariable].
    GlobalVariable(&'static GlobalVariable),
...
pub struct GlobalReferences {
    ...
    pub global_references: HashMap<Name, GlobalReference>,

    /// Maps module private [Function] or [GlobalVariable] name to an address.
    pub private_global_references: HashMap<ModuleHandle, HashMap<Name, GlobalReference>>,
...
impl GlobalReferences {
        ...
        M: Memory,
    {
        ...
        let mut create_fn_global_ref =
            |function: &'static Function| -> Result<GlobalReference, LLVMExecutorError> {
        ...
        let mut create_var_global_ref =
            |var: &'static GlobalVariable| -> Result<GlobalReference, LLVMExecutorError> {
    ...
    pub fn get(&self, name: &Name, module_handle: ModuleHandle) -> Option<&GlobalReference> {
