...
impl Project {
    ...
    fn from_modules(modules: Vec<Module>) -> Result<Self> {
    ...
    /// # }
    /// ```
    pub fn find_entry_function(&self, name: &str) -> Result<(ModuleHandle, &Function)> {
        ...
        return_function.ok_or_else(|| LLVMExecutorError::FunctionNotFound(name.to_string()))
    }
    ...
    pub fn get_function(
        ...
        if is_intrinsic(name) {
            if let Some(intrinsic) = self.intrinsics.get(name) {
                return Ok(FunctionType::Intrinsic(*intrinsic));
            }
        }
        ...
        Err(LLVMExecutorError::FunctionNotFound(f))
    }
    ...
    pub fn bit_size(&self, ty: &Type) -> Result<u32> {
        let size = size_in_bits(ty, self).ok_or(LLVMExecutorError::NoSize)?;
        Ok(size as u32)
    }

    /// Get the size in bytes of type `ty`.
    pub fn byte_size(&self, ty: &Type) -> Result<u32> {
        size_in_bytes(ty, self).map(|s| s as u32)
    }

    /// Get the offset to the index in bits for type `ty`.
    pub fn bit_offset(&self, ty: &Type, index: &DExpr, ctx: &DContext) -> Result<(DExpr, TypeRef)> {
        get_bit_offset(ty, index, self, ctx)
    }
    ...
    pub fn byte_offset(
        ...
        get_byte_offset(ty, index, self, ctx)
    }
